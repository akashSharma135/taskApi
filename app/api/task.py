...
def assign_task():
    ...
    db.tasks.insert_one({"id": unique_id(), "task": task_assigned,
                        "user_id": user_id, "manager_id": current_manager_id})
...
@task.route('/manager/delete-task', methods=['POST'])
@jwt_required()
def delete_task_manager():
    ...
    if row.get("manager_id") != get_jwt_identity():
        return jsonify(msg="You don't have any task to remove")
...
# user view task
@task.route('/user/tasks')
@jwt_required()
def user_tasks():
    # only user
    if not db.users.find_one({"id": get_jwt_identity()}):
        return jsonify(msg='protected route')

    row = db.tasks.find_one({"user_id": get_jwt_identity()})

    if not row:
        return jsonify(msg="No task to show")

    return jsonify({
        "task assigned": row.get('task'),
        "assigned by": db.managers.find_one({"id": row.get('manager_id')}).get('name')
    })
